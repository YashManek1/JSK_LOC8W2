...
export class GithubAnalyticsService {
  ...
  async syncRepoStats(identifier: string, newGithubUrl?: string) {
    ...
    let entry = await this.prisma.shortlistEntry.findFirst({
      where: isUuid
        ? { id: identifier }
        : { teamName: { equals: identifier, mode: 'insensitive' } },
      include: { config: true },
    });

    if (!entry) {
      // Auto-create missing team for Sandbox usage
      let config = await this.prisma.roundConfig.findFirst({
      ...
      this.logger.log(
        `Auto-created ShortlistEntry for newly submitted team ${identifier}`,
      );
    ...
    try {
      ...
      const repoResponse = await this.octokit.repos
        .get({ owner, repo })
        .catch(() => null);
      if (!repoResponse)
        throw new BadRequestException(
          `GitHub rejected access to ${owner}/${repo}. Is the repository Private, Typoed, or Empty?`,
        );

      const contributorsResponse = await this.octokit.repos
        .getContributorsStats({ owner, repo })
        .catch(() => null);
      const languagesResponse = await this.octokit.repos
        .listLanguages({ owner, repo })
        .catch(() => null);
      const punchCardResponse = await this.octokit.repos
        .getPunchCardStats({ owner, repo })
        .catch(() => null);

      // Fetch all branches to track multi-branch development
      const branchesResponse = await this.octokit.repos
        .listBranches({ owner, repo, per_page: 100 })
        .catch(() => null);
      const branches =
        branchesResponse?.data?.map((b) => ({
          name: b.name,
      ...
      if (contributorsResponse && contributorsResponse.status === 202) {
        this.logger.warn(
          `GitHub is generating stats for ${owner}/${repo}. Try again later.`,
        );
      ...
      const commitsResponse = await this.octokit.repos
      ...
      const timelineData =
        punchCardResponse && Array.isArray(punchCardResponse.data)
      ...
      if (entry.config?.problemStatement) {
        try {
          this.logger.log(
            `🤖 Starting AI Codebase Evaluation for ${entry.teamName}...`,
          );
          const repomixOutput = await this.github.fetchGitHubData(
            entry.githubUrl,
          );

          const aiEval =
            await this.groqSecondary.evaluateCodebaseImplementations(
          ...
          repoStats.missingPitchedFeatures =
            aiEval.missingPitchedFeatures || [];
          ...
          if (
            aiEval.developerMapping &&
            Array.isArray(aiEval.developerMapping)
          ) {
            repoStats.contributors = contributorsMap.map((c) => {
              const mapping = aiEval.developerMapping.find(
                (m: any) => m.author === c.author,
              );
          ...
          this.logger.log(
            `✅ AI Codebase Evaluation Complete for ${entry.teamName} (Relevance: ${aiEval.relevanceScore}%)`,
          );
        } catch (aiError: any) {
          this.logger.error(
            `⚠️ AI Codebase Evaluation failed: ${aiError.message}`,
          );
      ...
      this.logger.log(
        `✅ Synced Github cache for ${owner}/${repo} (${branches.length} branches tracked)`,
      );
      return repoStats;
    } catch (e: any) {
      ...
      throw new BadRequestException(
        `Could not fetch GitHub data: ${e.message}`,
      );
    }
  }
  ...
  async getRepoStats(identifier: string) {
    ...
    if (!entry) {
      throw new BadRequestException(
        `No team or entry found for identifier: ${identifier}`,
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `[getRepoStats] Failed to sync ${identifier}: ${err.message}`,
      );
      return {
        totalCommits: 0,
        contributors: [],
