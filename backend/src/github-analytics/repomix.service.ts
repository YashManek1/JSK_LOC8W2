...
export class RepomixService {
    ...
    async packRepository(githubUrl: string): Promise<string> {
        ...
        if (!githubUrl || !githubUrl.includes('github.com')) {
            throw new BadRequestException('Invalid GitHub URL provided to repomix service.');
        }
        ...
        try {
            ...
            throw new BadRequestException('Failed to pack codebase for analysis. Ensure the repository is public.');
        } finally {
            ...
            try {
                ...
                this.logger.error(`[${runId}] Failed to cleanup temp directory: ${tmpDir}`);
            }
        }
    }
    ...
    private detectPlagiarismSignatures(packedCode: string, repoUrl: string) {
        ...
        for (const sig of suspiciousSignatures) {
            if (packedCode.includes(sig)) {
                this.logger.warn(`🚩 PLAGIARISM FLAG: Boilerplate signature detected in repo ${repoUrl}. Pattern: "${sig}"`);
                signatureFound = true;
                break;
            }
        }
