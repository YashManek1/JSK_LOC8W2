...
export class ShortlistService {
    ...
    async upsertConfig(dto: any) {
        ...
        return this.db.roundConfig.create({ data: { ...data, isPublished: false } });
    }
    ...
    // ─── Single Submit ────────────────────────────────────────────────
    async createSubmission(data: { teamName: string; githubUrl?: string; pptxPath: string }) {
        const config = await this.getActiveConfig();
        if (!config) throw new Error('No active round configuration. Admin must set one up first.');
        ...
        await this.shortlistQueue.add('process', { entryId: entry.id }, { attempts: 2, backoff: { type: 'fixed', delay: 10000 } });
    ...
    async massCreateSubmissions(teams: { teamName: string; pptxPath: string }[]) {
        ...
        this.logger.log(`Mass upload: stored ${entries.length} teams as PENDING (not yet queued).`);
    ...
    async startMassEvaluation() {
        ...
        this.logger.log(`Started evaluation: queued ${pendingEntries.length} teams. ETA ~${eta}s`);
    ...
    async getLeaderboard() {
        const entries = await this.db.shortlistEntry.findMany({
            ...
            include: { config: { select: { isPublished: true, scoringWeights: true } } },
        });
        ...
        return entries.map((e: any, idx: number) => ({
            ...
            rank: idx + 1,
        })).sort((a: any, b: any) => (b.effectiveScore ?? 0) - (a.effectiveScore ?? 0))
    ...
    async eliminateEntry(id: string) {
        return this.db.shortlistEntry.update({ where: { id }, data: { status: 'ELIMINATED' } });
    }

    async restoreEntry(id: string) {
        return this.db.shortlistEntry.update({ where: { id }, data: { status: 'EVALUATED' } });
    }

    // ─── Admin Note ───────────────────────────────────────────────────
    async setAdminNote(id: string, note: string) {
        return this.db.shortlistEntry.update({ where: { id }, data: { adminNote: note } });
    }
    ...
    async requeueEntry(id: string) {
        await this.db.shortlistEntry.update({ where: { id }, data: { status: 'PENDING', failReason: null } });
    ...
    async publishLeaderboard() {
        ...
        for (const entry of evaluated) {
            await this.db.shortlistEntry.update({ where: { id: entry.id }, data: { rank } });
            rank++;
        }

        return this.db.roundConfig.update({ where: { id: config.id }, data: { isPublished: true } });
    }
    ...
    async rescoreAll() {
        ...
        const weights: Record<string, number> = (config.scoringWeights as any) || {};
        ...
        for (const entry of evaluated) {
            ...
            const newScore = this.groq.recomputeFinalScore(entry.pptScores as any, weights);
            await this.db.shortlistEntry.update({ where: { id: entry.id }, data: { finalScore: newScore } });
            updated++;
        }
        return { updated };
    }
    ...
    // ─── Funnel Stats ─────────────────────────────────────────────────
    async getStats() {
        const [total, pending, processing, evaluated, eliminated, failed] = await Promise.all([
